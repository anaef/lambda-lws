...
static lws_table_entry_t *lws_table_find(lws_table_t *t, lws_str_t *key, lws_uint_t hash);
static lws_table_entry_t *lws_table_insert(lws_table_t *t, lws_str_t *key, lws_uint_t hash);
...
lws_table_t *lws_table_create (size_t load) {
	...
	t = lws_calloc(sizeof(lws_table_t));
	if (!t) {
		return NULL;
	}
	...
	t->entries = lws_calloc(t->alloc * sizeof(lws_table_entry_t));
	if (!t->entries) {
		lws_free(t);
		return NULL;
	}
	lws_queue_init(&t->order);
	return t;
}
...
void lws_table_free (lws_table_t *t) {
	...
	if (t->dup || t->free) {
		while (!lws_queue_empty(&t->order)) {
			q = lws_queue_last(&t->order);
			entry = lws_queue_data(q, lws_table_entry_t, order);
...
void lws_table_clear (lws_table_t *t) {
	...
	lws_memzero(t->entries, t->alloc * sizeof(lws_table_entry_t));
}
...
void *lws_table_get (lws_table_t *t, lws_str_t *key) {
	lws_uint_t          hash;
	lws_table_entry_t  *entry;
	...
	if (t->capped) {
		lws_queue_remove(&entry->order);
		lws_queue_insert_tail(&t->order, &entry->order);
	}
	return entry->value;
}

int lws_table_set (lws_table_t *t, lws_str_t *key, void *value) {
	lws_uint_t          hash;
	lws_queue_t        *q;
	...
	if (value) {
		if (entry) {
			...
			if (t->free && value != entry->value) {
				lws_free(entry->value);
			}
			entry->value = value;
		} else {
			/* evict as needed */
			if (t->capped && t->count == t->cap) {
				q = lws_queue_head(&t->order);
				evict = lws_queue_data(q, lws_table_entry_t, order);
			...
			if (t->dup) {
				entry->key.data = lws_alloc(key->len);
				if (!entry->key.data) {
					return -1;
				}
				memcpy(entry->key.data, key->data, key->len);
...
int lws_table_next (lws_table_t *t, lws_str_t *key, lws_str_t **next, void **value) {
	lws_uint_t          hash;
	lws_queue_t        *q;
	lws_table_entry_t  *entry;
	...
	if (key) {
		...
		q = lws_queue_next(&entry->order);
	} else {
		/* start */
		q = lws_queue_head(&t->order);
	}
	if (q == lws_queue_sentinel(&t->order)) {
		return -1;
	}
	entry = lws_queue_data(q, lws_table_entry_t, order);
	*next = &entry->key;
...
static lws_uint_t lws_table_hash (lws_table_t *t, lws_str_t *key) {
	...
	if (t->ci) {
		while (p > key->data) {
			p--;
			hash ^= lws_tolower(*p);  /* no '--' due to macro multiple evaluation */
...
static size_t lws_table_size (lws_table_t *t, size_t size) {
	...
	if (lower >= lws_table_sizes_n) {
		lws_log(LWS_LOG_CRIT, "table size too large: %zu", size);
		return (size_t)-1;
	}
...
static int lws_table_rehash (lws_table_t *t, size_t alloc) {
	...
	lws_log_debug("table rehash t:%p old:%zu new:%zu", t, t->alloc, alloc_new);
	entries_new = lws_calloc(alloc_new * sizeof(lws_table_entry_t));
	if (!entries_new) {
		return -1;
	}
	...
	while (q != s) {
		...
		*entry_new = *entry;
		lws_queue_insert_head(&t->order, &entry_new->order);
		q = lws_queue_prev(q);
	}
	lws_free(entries);
	return 0;
...
static lws_table_entry_t *lws_table_find (lws_table_t *t, lws_str_t *key, lws_uint_t hash) {
	...
	while (entry->state != LWS_TES_UNUSED) {
		if (entry->hash == hash && entry->key.len == key->len && (t->ci
				? lws_strncasecmp(entry->key.data, key->data, key->len)
				: lws_strncmp(entry->key.data, key->data, key->len)) == 0) {
			return entry;
		}
		h = (h + q) % t->alloc;
		if (h == r) {
